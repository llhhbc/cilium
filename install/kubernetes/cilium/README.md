...
| installNoConntrackIptablesRules | bool | `true` | Install Iptables rules to skip netfilter connection tracking on all pod to pod traffic. This option is only effective when Cilium is running in direct routing and strict KPR mode. |
...
| operator.image | object | `{"awsDigest":"","azureDigest":"","genericDigest":"","pullPolicy":"Always","repository":"quay.io/cilium/operator","suffix":"","tag":"latest","useDigest":false}` | cilium-operator image. |
